import { Asset, applyParamsToScript, mConStr0, serializePlutusScript, stringToHex, Transaction } from "@meshsdk/core";
...
async function createProduct(
    ...
    try {
        ...
        const {
            utxos: utxoFarmer,
            walletAddress: addrFarmer,
            collateral: collateralFarmer,
        } = await getWalletInfoForTx(walletFarmer);
        ...
        const {
            utxos: utxoSMCCreate,
            walletAddress: addrSMCCreate,
            collateral: collateralSMCCreate,
        } = await getWalletInfoForTx(walletSMCCreate);
...
async function main() {
    ...
    const next_handler =
        "addr_test1qqwkave5e46pelgysvg6mx0st5zhte7gn79srscs8wv2qp5qkfvca3f7kpx3v3rssm4j97f63v5whrj8yvsx6dac9xrqyqqef6"; // địa chỉ người xử lí tiếp theo
