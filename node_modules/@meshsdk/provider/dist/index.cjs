...
var HydraProvider = class {
  ...
  constructor(url) {
    this._baseUrl = url;
    this._eventEmitter = new import_events.EventEmitter();
  ...
  resolveScriptRef = (kScriptRef) => {
    if (kScriptRef) {
      ...
      if (kScriptRef.type.startsWith("plutus")) {
        const normalized = (0, import_core_cst3.normalizePlutusScript)(kScriptRef.bytes, "DoubleCBOR");
        script = {
          code: normalized,
          version: kScriptRef.type.replace("plutus", "")
        };
      } else {
        script = (0, import_core_cst3.fromNativeScript)((0, import_core_cst3.deserializeNativeScript)(kScriptRef.bytes));
      }
      if (script) return (0, import_core_cst3.toScriptRef)(script).toCbor().toString();
    }
    return void 0;
  };
};
    ...
    try {
      ...
      if (status === 200) {
        ...
        return {
          ...
          VRFKey: import_core_cst4.VrfVkBech32.fromHex(data.vrf_key)
        };
      }
      throw parseHttpError(timestampedData);
    } catch (error) {
  ...
  resolveScript = (utxo) => {
    if (utxo.reference_script) {
      ...
      if (utxo.reference_script.type === "native") {
        ...
        if (scriptBytes) {
          const normalized = (0, import_core_cst4.normalizePlutusScript)(scriptBytes, "DoubleCBOR");
          script = {
            code: normalized,
      ...
      return (0, import_core_cst4.toScriptRef)(script).toCbor().toString();
    } else return void 0;
  };
};
...
var U5CProvider = class {
   ...
   */
  async fetchAddressUTxOs(address, asset) {
    const addressBytes = Buffer.from(import_core_cst5.Address.fromBech32(address).toBytes());
  ...
  _rpcUtxoToMeshUtxo(rpcTxoRef, rpcTxOutput) {
    ...
    return {
      ...
      output: {
        address: import_core_cst5.Address.fromBytes(
          import_core_cst5.CardanoSDKUtil.HexBlob.fromBytes(rpcTxOutput.address)
        ).toBech32(),
        amount,
        dataHash,
        plutusData,
...
var YaciProvider = class {
  ...
  async fetchAccountInfo(address) {
    const rewardAddress = address.startsWith("addr") ? (0, import_core_cst6.resolveRewardAddress)(address) : address;
    try {
      const { data, status } = await this._axiosInstance.get(
        `accounts/${rewardAddress}`
      );
  ...
  resolveScriptRef = async (scriptHash) => {
    if (scriptHash) {
      ...
      if (status === 200) {
        ...
        if (data.type.startsWith("plutus")) {
          const plutusScript = await this.fetchPlutusScriptCBOR(scriptHash);
          const normalized = (0, import_core_cst6.normalizePlutusScript)(plutusScript, "DoubleCBOR");
          script = {
            version: data.type.replace("plutus", ""),
            code: normalized
          };
        ...
        return (0, import_core_cst6.toScriptRef)(script).toCbor();
      }
      throw parseHttpError(data);
    }
    return void 0;
