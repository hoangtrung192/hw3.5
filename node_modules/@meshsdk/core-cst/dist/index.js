...
var CoseSign1 = class _CoseSign1 {
  ...
  getAddress() {
    const address = this.protectedMap.map.find((value) => {
      return import_json_bigint.default.stringify(value.k) === import_json_bigint.default.stringify(new CborText("address"));
    });
  ...
  getPublicKey() {
    const publicKey = this.protectedMap.map.find((value) => {
      return import_json_bigint.default.stringify(value.k) === import_json_bigint.default.stringify(new CborUInt(4));
    });
...
var getPublicKeyFromCoseKey = (cbor) => {
  const decodedCoseKey = Cbor.parse(cbor);
  const publicKeyEntry = decodedCoseKey.map.find((value) => {
    return import_json_bigint.default.stringify(value.k) === import_json_bigint.default.stringify(new CborNegInt(BigInt(-2)));
  });
  if (publicKeyEntry) {
...
var resolveTxHash = (txHex) => {
  ...
  return Cardano2.TransactionId.fromHexBlob(HexBlob7.fromBytes(hash2)).toString();
};
...
import { Serialization as Serialization7, setInConwayEra } from "@cardano-sdk/core";
  ...
  switch (voter.type) {
    case "ConstitutionalCommittee": {
      switch (voter.hotCred.type) {
        case "KeyHash": {
          return Serialization6.Voter.newConstitutionalCommitteeHotKey({
            type: 0,
    ...
    case "DRep": {
      ...
      if (cardanoDrep.toKeyHash() !== void 0) {
        return Serialization6.Voter.newDrep({
          type: 0,
    ...
    case "StakingPool": {
      return Serialization6.Voter.newStakingPool(
...
var toCardanoVotingProcedure = (votingProcedure) => {
  return new Serialization6.VotingProcedure(
...
var toCardanoAnchor = (anchor) => {
  return new Serialization6.Anchor(
    anchor.anchorUrl,
...
var CardanoSDKSerializer = class {
  ...
  resolver = {
    ...
    tx: {
      resolveTxHash: function(txHex) {
        return Transaction.fromCbor(Serialization7.TxCBOR(txHex)).getId();
      }
    },
    data: {
  ...
  addSigningKeys = (txHex, signingKeys) => {
    let cardanoTx = Transaction.fromCbor(Serialization7.TxCBOR(txHex));
...
var CardanoSDKSerializerCore = class {
  ...
  constructor(protocolParams) {
    ...
    this.txBody = new TransactionBody(
      Serialization7.CborSet.fromCore([], TransactionInput.fromCore),
      [],
      BigInt(0),
      void 0
    );
  ...
  addCert = (cert, index) => {
    const currentCerts = this.txBody.certs() ?? Serialization7.CborSet.fromCore([], Serialization7.Certificate.fromCore);
  ...
  addCollateralInput = (collateral) => {
    ...
    const collateralInputs = this.txBody.collateral() ?? Serialization7.CborSet.fromCore([], TransactionInput.fromCore);
  ...
  addAllRequiredSignatures = (requiredSignatures) => {
    const requiredSigners = this.txBody.requiredSigners() ?? Serialization7.CborSet.fromCore([], Serialization7.Hash.fromCore);
    let requiredSignerValues = [...requiredSigners.values()];
    for (const requiredSigner of requiredSignatures) {
      requiredSignerValues.push(
        Serialization7.Hash.fromCore(Ed25519KeyHashHex2(requiredSigner))
      );
    }
  ...
  addMetadata = (metadata) => {
    this.txAuxilliaryData.setMetadata(
      new Serialization7.GeneralTransactionMetadata(
        toCardanoMetadataMap(metadata)
      )
    );
  };
  buildWitnessSet = () => {
    let nativeScripts = this.txWitnessSet.nativeScripts() ?? Serialization7.CborSet.fromCore([], NativeScript.fromCore);
    let v1Scripts = this.txWitnessSet.plutusV1Scripts() ?? Serialization7.CborSet.fromCore([], PlutusV1Script.fromCore);
    let v2Scripts = this.txWitnessSet.plutusV2Scripts() ?? Serialization7.CborSet.fromCore([], PlutusV2Script.fromCore);
    let v3Scripts = this.txWitnessSet.plutusV3Scripts() ?? Serialization7.CborSet.fromCore([], PlutusV3Script.fromCore);
    ...
    let datums = this.txWitnessSet.plutusData() ?? Serialization7.CborSet.fromCore([], PlutusData.fromCore);
    ...
    let costModelV1 = Serialization7.CostModel.newPlutusV1(
      DEFAULT_V1_COST_MODEL_LIST
    );
    let costModelV2 = Serialization7.CostModel.newPlutusV2(
      DEFAULT_V2_COST_MODEL_LIST
    );
    let costModelV3 = Serialization7.CostModel.newPlutusV3(
      DEFAULT_V3_COST_MODEL_LIST
    );
    let costModels = new Serialization7.Costmdls();
  ...
  createDummyTx = (numberOfRequiredWitnesses) => {
    let dummyWitnessSet = TransactionWitnessSet.fromCbor(
      HexBlob9(this.txWitnessSet.toCbor())
    );
    ...
    dummyWitnessSet.setVkeys(
      Serialization7.CborSet.fromCore(dummyVkeyWitnesses, VkeyWitness.fromCore)
    );
  ...
  addBasicVote = (basicVote) => {
    const votes = this.txBody.votingProcedures() ?? Serialization7.VotingProcedures.fromCore([]);
    votes.insert(
...
export {
  ...
  ScriptHash,
  ScriptPubkey,
  Serialization8 as Serialization,
  Slot,
