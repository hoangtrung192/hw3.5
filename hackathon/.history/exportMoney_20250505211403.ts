...
async function exportMoney(
  ...
  try {
    ...
    // Xử lý mỗi UTXO script riêng biệt
    for (const tx of txHash) {
      ...
      // CHÚ Ý: Gọi spendingPlutusScriptV3() cho MỖI script input
      txBuilder
        ...
        .txInScript(contributeScriptCbor); // Thêm script CBOR cho MỖI script input
    }
     const amountReverse = cash - amount;
    // Thêm collateral MỘT LẦN DUY NHẤT sau vòng lặp
     txBuilder
     .spendingPlutusScriptV3()
      .txInCollateral(
    ...
    // Thêm output cho người nhận (không cần gọi spendingPlutusScriptV3 ở đây)
    ...
    // Nếu cần, thêm output trả lại script
    ...
    // Hoàn thiện giao dịch (không cần gọi spendingPlutusScriptV3 ở đây)
    txBuilder
      .changeAddress(walletAddress)
      .requiredSignerHash(pubkeyContributor)
      .requiredSignerHash(pubkeyAdmin) // Thêm cả admin
